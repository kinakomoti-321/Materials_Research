...
bool loadObj(const std::string &filepath, const std::string &filename, MR::ModelInfo &model)
{
    ...
    if (has_material)
    {
        for (int i = 0; i < load_materials.size(); i++)
        {
            ...
            model.materials.push_back(std::make_shared<MR::Diffuse_Lambert>(load_materials[i].name, mat));
        }
    }
    else
    {
        ...
        mat.emmision = vec3(0);
        model.materials.push_back(std::make_shared<MR::Diffuse_Lambert>("Sample", mat));
    }
